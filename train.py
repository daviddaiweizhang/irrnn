...
def get_model(
        ...
        epochs=None, load_existing=True, device=None, **kwargs):
    checkpoint_file = prefix + 'model.pt'
    history_file = prefix + 'history.pickle'

    # load model if exists
    if load_existing and os.path.exists(checkpoint_file):
    ...
    if (epochs is not None) and (epochs > 0):
        model, hist, trainer = train_model(
            ...
            dataset=dataset, epochs=epochs, device=device, **kwargs)
...
def train_model(
    ...
    if device is None:
        ...
        accelerator_dict = {'cuda': 'gpu', 'cpu': 'cpu'}
